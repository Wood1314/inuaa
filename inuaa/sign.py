...
def login(login_id, login_password, imei, mobile_type):
    '''
    ...
    for _ in range(try_times):
        try:
            time.sleep(delay)
            r = requests.get('https://m.nuaa.edu.cn/a_nuaa/api/login-v2/index', cookies=cookies,
        ...
        try:
            data = {
                ...
                'geo_api_info': {"type": "complete",
                                 "position": {"Q": 31.939607, "R": 118.791155, "lng": 118.791155, "lat": 31.939607},
                                 "location_type": "html5",
                                 "message": "Get geolocation success.Don't need convert.Get address success.",
                                 "accuracy": 29, "isConverted": 1, "status": 1,
                                 "addressComponent": {"citycode": "025", "adcode": "320115", "businessAreas": [
                                     {"name": "开发区", "id": "320115",
                                      "location": {"Q": 31.925973, "R": 118.80980399999999, "lng": 118.809804,
                                                   "lat": 31.925973}}], "neighborhoodType": "", "neighborhood": "",
                                                      "building": "", "buildingType": "", "street": "将军大道",
                                                      "streetNumber": "29号", "country": "中国", "province": "江苏省",
                                                      "city": "南京市", "district": "江宁区",
                                                      "towncode": "320115011000", "township": "秣陵街道"},
                                 "formattedAddress": "江苏省南京市江宁区秣陵街道慧园路南京航空航天大学将军路校区",
            ...
            if r.text.find('成功') >= 0:
                ...
                if user['receiver_mail'] != '':
                    send_mail(mail_username, mail_password, smtp_host,
                              user['receiver_mail'], user['name'] + '打卡成功', '打卡成功', user['name'], '打卡机器人')
                return True
            else:
                print('打卡失败，尝试重新登陆')
